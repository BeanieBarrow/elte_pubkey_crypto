...
\begin{itemize}
	...
	\item Example: Ali Baba's cave: Bob can only meet Alice from $A$ when entering from $B$ if he knows the password. Alice does not hear the password.
...
\begin{definition}[Random Oracle Model (ROM)]
	For a given input $x$, if it has not yet been queried, then let $r \in_R \mathbb{Z}_q$, and put $r = \mathcal{O}(x)$, where $\mathcal{O}$ is the random oracle. If it has been queried, return the same output.
\end{definition}
