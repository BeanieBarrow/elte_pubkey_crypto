...
\begin{document}
	...
	\begin{proof}
		Let $\sigma = ord_p (g)$
		\begin{align}
			p-1 = \sigma \cdot g + r && 0 \le r \le \sigma \\
			1 = g^{p-1} = g^{\sigma \cdot g + r} = && \text{(by Fermat's little theorem)} \\
			= (g^{\sigma})^g \cdot g^r && \text{(where $g^\sigma$ comes from the definition of the order of $g$)}
		\end{align}
